...
io.write('\n\27[1;35m⬇Send UserName For Sudo : ارسل معرف Bot Dev 🎖...\n\27[0;39;49m')
local User_Sudo = io.read()
if User_Sudo ~= '' then
io.write('\n\27[1;31m◦ The UserNamr Is Saved : تم حفظ معرف Dev  واستخراج ايدي Dev \n\27[0;39;49m')
...
io.write('\n\27[1;35m⬇Send id For Sudo : ارسل ايدي Bot Dev 🎖...\n\27[0;39;49m')
local User_Sudo = io.read()
if User_Sudo ~= '' then
io.write('\n\27[1;31m◦ The id Is Saved : تم حفظ ايدي Dev  واستخراج ايدي Dev \n\27[0;39;49m')
...
Status = "انا البوت :) "
elseif redis:sismember(bot_id.."NightRang:Developer:Bot1", user_id) then
Status = redis:get(bot_id.."NightRang:Developer:Bot:Reply"..chat_id) or "Dev 🎖"  
elseif redis:sismember(bot_id.."NightRang:Developer:Bot", user_id) then
Status = redis:get(bot_id.."NightRang:Developer:Bot:Reply"..chat_id) or "Dev "  
elseif redis:sismember(bot_id.."NightRang:President:Group"..chat_id, user_id) then
...
Text = [[
شيك على السورس و مطور السورس اذا عجبك نصب لك بوت عنده @BQQQC 
]]
...
send(msg.chat_id_,msg.id_,'• تم الغاء حفظ كليشة Dev ')
return false
end
redis:set(bot_id..'NightRang:Texting:DevSlbotss',text)
redis:del(bot_id..'NightRang:GetTexting:DevSlbotss'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'• تم حفظ كليشة Dev ')
send(msg.chat_id_,msg.id_,text)
return false
end
...
local Text_keyboard = '• اهلا بك عزيزي Bot Dev 🎖\n في اوامرك الخاصه \n يمكنك تحكم في بوت عن طريق كيبورد ادناه '
local List_keyboard = {
{'تغيير اسم البوت'},
{'الاحصائيات'},
{'تفعيل تواصل','تعطيل تواصل'},
{'الاذاعه'},
{'مسح قائمه Dev','مسح قائمه Dev🎖'},
...
send(msg.chat_id_,msg.id_,' هذا الامر خاص Bot Dev 🎖فقط')
return false
end
...
send(msg.chat_id_, msg.id_,'•  تم حذف كليشه Dev ')
end
...
send(msg.chat_id_, msg.id_, "•  تم مسح قائمه Dev من البوت  ")
end
if text == ("مسح قائمه Dev🎖") then
redis:del(bot_id.."NightRang:Developer:Bot")
send(msg.chat_id_, msg.id_, "•  تم مسح قائمه Dev من البوت  ")
...
if text == ("قائمه Dev") and Dev_Bots(msg) then
local list = redis:smembers(bot_id.."NightRang:Developer:Bot")
if #list == 0 then
return send(msg.chat_id_, msg.id_, "• لا يوجد Dev ")
end
Sudos = "\n• قائمه Devommander  في البوت \n━━━━━━━━\n"
for k,v in pairs(list) do
tdcli_function ({ID = "GetUser",user_id_ = v},function(arg,data) 
if data.username_ then
username = '[@'..data.username_..']'
...
send(msg.chat_id_, msg.id_, "لا تستطيع حظر Bot Dev 🎖عام")
return false 
end
...
if Dev_Bots_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "لا تستطيع كتم Bot Dev 🎖عام")
return false 
end
if DeveloperBot12(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "لا تستطيع كتم Bot Dev 🎖عام")
...
if Dev_Bots_User(result.id_) == true then
send(msg.chat_id_, msg.id_, " Bot Dev 🎖لا تستطيع تقييد عام")
return false 
end
...
if text == ("رفع Dev") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Bots(msg) then
function FunctionStatus(arg, result)
redis:sadd(bot_id.."NightRang:Developer:Bot", result.sender_user_id_)
Send_Options(msg,result.sender_user_id_,"reply","تم ترقيته Dev في البوت")  
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end

if text == ("تنزيل Dev") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Bots(msg) then
function FunctionStatus(arg, result)
...
if text and text:match("^رفع Dev @(.*)$") and Dev_Bots(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
send(msg.chat_id_,msg.id_,"عذرا هاذا معرف قناة")   
...
Send_Options(msg,result.id_,"reply","تم ترقيته Dev في البوت")  
else
send(msg.chat_id_, msg.id_,"المعرف غلط ")
end
end
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^رفع Dev @(.*)$")}, FunctionStatus, nil)
end

if text and text:match("^تنزيل Dev @(.*)$") and Dev_Bots(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^تنزيل Dev @(.*)$")}, FunctionStatus, nil)
end
...
if text == ("قائمه Dev") and DeveloperBot(msg) then
local list = redis:smembers(bot_id.."NightRang:Developer:Bot")
if #list == 0 then
return send(msg.chat_id_, msg.id_, "• لا يوجد Dev ")
end
...
if text == ("قائمه Dev🎖") and DeveloperBot1(msg) then
local list = redis:smembers(bot_id.."NightRang:Developer:Bot1")
if #list == 0 then
return send(msg.chat_id_, msg.id_, "• لا يوجد Dev ")
end
...
if text == ("مسح قائمه Dev") and Dev_Bots(msg) then
redis:del(bot_id.."NightRang:Developer:Bot")
send(msg.chat_id_, msg.id_, "•  تم مسح قائمه Dev من البوت  ")
end
if text == ("مسح قائمه Dev🎖") and Dev_Bots(msg) then
...
send(msg.chat_id_, msg.id_,"-") 
send(GP_ID[2], 0,"•  تم مغادرة المجموعه بامر من Dev البوت") 
send(msg.chat_id_, msg.id_,"•  تم مغادرة المجموعه بامر من Dev البوت") 
...
if text and text:match("^تغيير رد Dev  (.*)$") and Owner(msg) then

local Teext = text:match("^تغيير رد Dev  (.*)$") 
redis:set(bot_id.."NightRang:Developer:Bot:Reply"..msg.chat_id_,Teext)
send(msg.chat_id_, msg.id_,"•  تم تغيير رد Dev  الى :"..Teext)
return false end
...
return false end
if text == 'حذف رد Dev ' and Owner(msg) then

redis:del(bot_id.."NightRang:Developer:Bot:Reply"..msg.chat_id_)
send(msg.chat_id_, msg.id_,"• تم حدف رد Dev ")
return false end
...
send(msg.chat_id_, msg.id_,"• تم تعطيل الاذاعه من قبل Bot Dev 🎖!")
return false end
redis:setex(bot_id.."NightRang:Broadcasting:Users" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
send(msg.chat_id_, msg.id_,"• ارسل لي المنشور الان\n• يمكنك ارسال -{ صوره - ملصق - متحركه - رساله }\n• لالغاء الاذاعه ارسل : الغاء") 
return false
...
send(msg.chat_id_, msg.id_,' ارسل الان معرف Bot Dev 🎖الجديد')
return false
end
...
redis:del(bot_id..'Set:Text:Dev:Bot:id'..msg.chat_id_)
send(msg.chat_id_, msg.id_,'تم تغيير Bot Dev 🎖\n الرجاء ارسل امر [تحديث]')
dofile('Info_Sudo.lua')  
return false
end
